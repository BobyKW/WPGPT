...
class Comments_Provider extends Base_Ability_Provider {
    ...
    public function get_abilities(): array {
        return array(
            ...
            'wpgpt/discussion-audit' => array('label'=>__('Discussion audit','wpgpt-mcp-bridge'),'description'=>__('Lista posts y páginas con comentarios o pings abiertos, con override respecto a la configuración global.','wpgpt-mcp-bridge'),'input_schema'=>$this->discussion_audit_schema(),'execute_callback'=>array($this,'discussion_audit'),'output_schema'=>$this->object_schema(),'permission_callback'=>array($this,'can_edit_content')),
    ...
    public function discussion_audit(array $input){ return $this->service()->discussion_audit($input);} public function comment_query(array $input){ return $this->service()->query_comments($input);} public function comment_get(array $input){ return $this->service()->get_comment($input);} public function comment_upsert(array $input){ return $this->service()->upsert_comment($input);} public function comment_status_bulk(array $input){ return $this->service()->bulk_status($input);} public function comment_delete(array $input){ return $this->service()->delete_comment_data($input);} private function service(): Comments_Service { return $this->service ??= new Comments_Service(); }
    private function discussion_audit_schema(): array { return array('type'=>'object','properties'=>array('post_type'=>array('type'=>'string'),'post_status'=>array('type'=>'string'),'limit'=>array('type'=>'integer'),'override_only'=>array('type'=>'boolean'),'comments_open'=>array('type'=>'boolean'),'pings_open'=>array('type'=>'boolean'))); }
